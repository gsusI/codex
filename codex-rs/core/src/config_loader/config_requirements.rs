...
impl From<ConfigRequirementsToml> for ConfigRequirementsWithSources {
    fn from(toml: ConfigRequirementsToml) -> Self {
        // Destructure without `..` so adding fields to `ConfigRequirementsToml`
        // forces this conversion to be updated.
...
impl TryFrom<ConfigRequirementsToml> for ConfigRequirements {
    ...
    fn try_from(toml: ConfigRequirementsToml) -> Result<Self, Self::Error> {
        ConfigRequirementsWithSources::from(toml).try_into()
    }
}
